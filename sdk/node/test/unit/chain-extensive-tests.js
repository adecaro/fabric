...
function getUser(chain, name, cb) {
...
test('Chain initialization', function (t) {
    t.plan(1);

    var chain = hlc.newChain("testChain");

    // Test against an uninitialized chain object    
    
    chain.getMember("dummy_user", function(err){
        if (!err) {
            t.fail("Chain not initialized. 'chain.getMember' should fail.")
        }    
    });

    chain.getUser("dummy_user", function(err){
        if (!err) {
            t.fail("Chain not initialized. 'getUser' should fail. ")
        }
    });

    chain.register("dummy_user", function(err){
        if (!err) {
            t.fail("Chain not initialized. 'getUser' should fail." )
        }
    });
            ...
            WebAppAdmin.enroll(pw, function (err, crypto) {
                if (err) {
                    ...
                    if (!WebAppAdmin.isEnrolled()) {
                        t.fail("(After Enroll) 'WebAppAdmin' member is not enrolled. It should.");
                    } else {
                        t.pass("(After Enroll) 'WebAppAdmin' member is enrolled as expected.");
                    }
                    ...
                    // Try to reload the member and check if it is enrolled
                    var tmpChain = hlc.newChain("Enroll WebAppAdmin");
                    tmpChain.setKeyValStore(hlc.newFileKeyValStore('/tmp/keyValStore'));
                    if (fs.existsSync("tlsca.cert")) {
                        tmpChain.setMemberServicesUrl("grpcs://localhost:50051",  fs.readFileSync('tlsca.cert'));
                    } else {
                        tmpChain.setMemberServicesUrl("grpc://localhost:50051");
                    }
                    tmpChain.addPeer("grpc://localhost:30303");
                    tmpChain.setDevMode(true);
                    tmpChain.getMember("WebAppAdmin", function (err, WebAppAdmin) {
                        if (err) {
                            t.fail("Failed to get WebAppAdmin member " + " ---> " + err);
                            t.end(err);
                        } else {
                            if (!WebAppAdmin.isEnrolled()) {
                                t.fail("'WebAppAdmin' member is not enrolled. It should.")
                            } else {
                                t.pass("'WebAppAdmin' member is enrolled as expected.");
                            }
                        }
                    });
...
test('Register and enroll a new user', function (t) {
    ...
    var chain = hlc.getChain("testChain");
    getUser(chain, test_user1.name, function (err, user) {
        if (err) {
            ...
            // Try to reload the member and check if it is enrolled
            var tmpChain = hlc.newChain("Register and enroll a new user");
            tmpChain.setKeyValStore(hlc.newFileKeyValStore('/tmp/keyValStore'));
            if (fs.existsSync("tlsca.cert")) {
                tmpChain.setMemberServicesUrl("grpcs://localhost:50051",  fs.readFileSync('tlsca.cert'));
            } else {
            ...
            tmpChain.addPeer("grpc://localhost:30303");
            tmpChain.setDevMode(true);
            tmpChain.getMember(test_user1.name, function (err, u) {
                if (err) {
                    t.fail("Failed to get WebAppAdmin member " + " ---> " + err);
                    t.end(err);
                } else {
                    if (!u.isEnrolled()) {
                        t.fail(test_user1.name + " member is not enrolled. It should.");
                        t.end()
                    } else {
                        test_user_Member1 = u;

                        t.pass(test_user1.name + " member is enrolled as expected.");
                    }
                }
            });
        }
